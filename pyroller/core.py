...
class KeepDropTag:
    ...
    def __repr__(self) -> str:
        ...
        if self:
            attr_strings = [
                f'{key}={value}' for key, value in zip(self.__dict__.keys(),
...
class RerollExplodeTag:
    ...
    def __repr__(self) -> str:
        ...
        if self:
            ...
            string_out = string_out + f'(once={self.once}, ' + \
                ', '.join(attr_strings) + ')'
        return string_out
...
class Roll:
    ...
    def __init__(self, roll_string: str):
        ...
        if self.reroll and (invalid_reroll_value or
                            self.reroll.under > self.num_faces):
            raise ValueError(
        ...
        if self.explode and (invalid_explode_value or
                             self.explode.under > self.num_faces):
            raise ValueError(
    ...
    def simulate(self,
                 num: int = DEFAULT_NUM_SIMULATIONS) -> 'RollStatsSimulated':
        ...
        for ind_roll in range(num):
            ...
            while ind < (self.num_dice + num_explosions):
                ...
                if self.reroll and not (num_rerolls > 0 and self.reroll.once):
                    while tmp_roll == self.reroll.value or tmp_roll > self.reroll.over or tmp_roll < self.reroll.under:
                        num_rerolls += 1
                        tmp_roll = random.randint(1, self.num_faces)
                        if debug:
                ...
                explode_test = self.explode and not (num_explosions > 0 and
                                                     self.explode.once)
                if explode_test and (tmp_roll == self.explode.value or
                                     tmp_roll > self.explode.over or
                                     tmp_roll < self.explode.under):
                    num_explosions += 1
                    if debug:
...
class RollStats:
    ...
    def __init__(self, roll: ('Roll', str)):
    ...
    def __str__(self) -> str:
        base_string = f'{self.__class__.__name__}:\n' + \
    ...
    def __init__(self, roll: ('Roll', str)):
        ...
        if self.roll.reroll and not self.roll.reroll.once:
            ...
            if self.roll.reroll.over < self.roll.num_faces:
                ...
                max_die_value = self.roll.num_faces - 1  # Assuming that there are at least two faces
            else:
        ...
        if self.roll.reroll or self.roll.explode:
            print('Warning: reroll/explode statistics TBD')
            return
        if self.roll.keep:
            print('Warning: keep high/low statistics TBD')
            return
        if self.roll.drop:
            print('Warning: drop high/low statistics TBD')
            return

        result_keys = list(range(self.min, self.max + 1))

        # Calculate the individual probabilities
        single_die_prob = np.array([1 / self.roll.num_faces] *
                                   self.roll.num_faces,
                                   dtype=float)
        result_values = single_die_prob.copy()
        for _ in range(1, self.roll.num_dice):
            tmp_array = result_values[np.newaxis].T @ single_die_prob[
                np.newaxis]
            tmp_list = [
...
class RollStatsSimulated(RollStats):
    ...
    def __init__(self, roll: ('Roll', str), num: int = DEFAULT_NUM_SIMULATIONS):
        # Call parent class constructor
        super().__init__(roll)
...
def win(roll: ('Roll', 'RollStatsCalculated', 'RollStatsSimulated', str),
        target: int) -> float:
...
def lose(roll: ('Roll', 'RollStatsCalculated', 'RollStatsSimulated', str),
         target: int) -> float:
